...
@section('content')
  @php
    $activeTA = \App\Models\TahunAjar::where('aktif', true)->first();
    $isOtherActive = $activeTA && $activeTA->id !== $tahunAjar->id;
  @endphp
      ...
      <div class="w-full max-w-full px-3 lg:w-8/12">
        ...
        <div class="relative flex flex-col min-w-0 break-words
          ...
          <div class="p-6 pb-0 mb-0 rounded-t-2xl">
            <h6 class="dark:text-white">Edit Tahun Ajar</h6>
          </div>
          ...
          <div class="flex-auto p-6">
            ...
            <form action="{{ route('admin.tahun-ajar.update', $tahunAjar->id) }}" method="POST">
              ...
              <div class="mb-4">
                ...
                <input type="text" name="tahun" value="{{ old('tahun', $tahunAjar->tahun) }}"
                  placeholder="Contoh: 2024 / 2025" class="focus:shadow-primary-outline
              ...
              <div class="flex items-center my-6">
                ...
                <div class="block pl-0 ml-auto min-h-6">
                  <input type="checkbox" name="aktif" {{ old('aktif', $tahunAjar->aktif) ? 'checked' : '' }} class="rounded-10 duration-250 ease-in-out
              ...
              @if ($isOtherActive)
                <p class="mt-2 text-xs text-yellow-600 dark:text-yellow-400">
                  ⚠ Tahun Ajar <strong>{{ $activeTA->tahun }}</strong> sedang aktif.
                </p>
              @endif

              <!-- ACTION -->
              <div class="flex justify-end gap-3">
                <a href="{{ route('admin.tahun-ajar.index') }}" class="px-4 py-2 text-xs font-semibold
                            text-slate-600 dark:text-white
                            hover:underline">
                  Kembali
                </a>

                <button type="submit" class="px-6 py-2 text-xs font-semibold
